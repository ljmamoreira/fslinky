...
elástica e gravítica do modelo numérico.
	...
	\begin{figure}
		...
		\begin{tikzpicture}
			...
			\addplot[mylightgreen, loosely dotted, line width=1mm] table[x = t, y = X1] {Lab.dat};
			\addlegendentry[UBI]{Massa $0$};
			\addplot[p5, loosely dotted, line width=1mm] table[x = t, y = X2] {Lab.dat};
			\addlegendentry[UBI]{Massa $1$};
			\end{axis}
		\end{tikzpicture}
		\caption{\label{fig:a} Deslocamentos a partir das posições de equilíbrio na queda de uma mola com 3 e 2 massas, a tracejado estão as curvas observadas e a contínuo estão as soluções numéricas.}
	\end{figure}
